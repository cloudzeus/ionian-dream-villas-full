...
export default function HeroSlideshow({ locale, villas, slides }: Props) {
  ...
  const images: { src: string; caption: string }[] =
    slides.length > 0
      ? slides.map(s => ({ src: s.imageUrl, caption: locale === "gr" ? s.captionGr : s.captionEn }))
  ...
  return (
    <section className="x-hero-section" style={{ position: "relative", height: "100vh", minHeight: 640, overflow: "hidden", color: "white" }}>
      {/* Slide backgrounds */}
      <div className="x-hero-bg" style={{ position: "absolute", inset: "-15% 0", background: "linear-gradient(135deg, #1C3848 0%, #0e1e28 100%)" }}>
        {images.map((img, i) => (
          <Image key={i} src={img.src} alt="Lefkada · Ionian Sea" fill priority={i === 0} sizes="100vw"
            style={{ objectFit: "cover", objectPosition: "center 40%", transition: "opacity 1.4s ease", opacity: i === active ? 1 : 0 }}
          />
        ))}
      </div>
      ...
      {/* Top bar */}
      <div className="x-hero-topbar" style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 3, padding: "100px 48px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      ...
      {/* Bottom bar */}
      <div className="x-hero-bottombar" style={{ position: "absolute", left: 48, right: 48, bottom: 48, zIndex: 3, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 16 }}>
          ...
          <Link href={`/${locale}/villas`} className="x-hero-meta btn-hover-ghost"
            style={{ display: "inline-flex", alignItems: "center", gap: 12, border: "1px solid rgba(255,255,255,0.35)", color: "white", padding: "14px 28px", textDecoration: "none", fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", backdropFilter: "blur(8px)", background: "rgba(255,255,255,0.07)", transition: "background 0.3s ease, border-color 0.3s ease" }}
          >
            Explore the villas
      ...
      {/* Scroll indicator */}
      <div className="x-hero-scroll x-hero-meta" style={{ position: "absolute", right: 48, top: "50%", transform: "translateY(-50%)", zIndex: 3, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
        <div style={{ writingMode: "vertical-rl", fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)" }}>Scroll</div>
