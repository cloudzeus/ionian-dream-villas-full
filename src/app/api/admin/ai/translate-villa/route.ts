...
export async function POST(req: NextRequest) {
  try {
    ...
    const translated = await deepseekJSON(
      `You are a professional translator for a luxury villa rental website in Lefkada, Greece. Translate JSON fields from English to ${lang}. Return ONLY valid JSON with the same keys. Keep "nameLocal" as the Greek local name. Keep "view" as a dot-separated list. Match luxury/hospitality tone.`,
