...
export async function POST(req: NextRequest) {
  try {
    ...
    const translated = await deepseekJSON<Record<string, string>>(
      `You are a professional translator for a luxury villa rental website in Lefkada, Greece. Translate the provided JSON key-value pairs from English to ${lang}. Return ONLY valid JSON with the exact same keys. Preserve tone: headings should be concise, mottos should be poetic, button labels should be short and actionable. For Greek translations of "closingGreek" keep it as authentic Greek phrasing.`,
